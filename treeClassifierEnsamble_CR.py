...
def dropcol_importances(rf, X_train, y_train):
	...
	imp = np.array(imp)
	return sorted(
		zip(X_train.columns, imp, range(len(X_train.columns))),
		reverse=True,
		key=lambda kv: kv[1],
	)
...
def main():
	...
	data_predict.drop(
		"HardVotePred", inplace=True, axis=1
	)  # dropping the column with the result of the classifier used for cam ready submission (in github)
	pred_cols = data_predict.columns
	# print pred_cols # To check all columns of prediction dataset.
	data_predict[pred_cols] = data_predict[pred_cols].apply(
	...
	SMs = [
		...
		smt,
		tl,
	] + cc_models  # This array encodes the order for accessing balancing techniques
	without_asn = [2, 3]
	...
	models_reps = [0, 10, 10, 10, 4] + [0] * len(
		cc_models
	)  # Picking number of ensembles and technique to be used to balance training set for those ensembles (following the SMs list line 83)
	for i, sm in enumerate(SMs):
		...
		for r in range(rep):
			...
			if i in without_asn:
				...
				X, y = (
					data_balanced.drop(["ASN", "class"], axis=1),
					data_balanced["class"],
				)
			...
			data_balanced.loc[:, "oob_class_pred"] = [
				forest.classes_[x]
				for x in np.argmax(forest.oob_decision_function_, axis=1)
			]
			data_balanced.loc[
				:, "class_%d" % forest.classes_[0]
			] = forest.oob_decision_function_[:, 0]
			data_balanced.loc[
				:, "class_%d" % forest.classes_[1]
	...
	hard_vote_pred = np.asarray(
		[clf[1].predict(data_predict.drop("ASN", axis=1)) for clf in estimators_list]
	).T
